...
namespace CustomEditorTools
{
    ...
    {
        //GUI related
        private GameObject specularProbesParent;
        private static string emissivesParentName = "SpecularProbesEmissives";
        private static int guiSectionSpacePixels = 10;
        private Vector2Int windowSize = new Vector2Int(350, 510);
        ...
        void OnGUI()
        {
            ...
            if (GUILayout.Button("Spawn Emissives"))
            {
                PlaceEmissives(true);
            }
        ...
        private void BakeReflectionProbes()
        {
            ...
            EditorUtility.DisplayProgressBar("Baking Specular Probes", "Assigning Baked Reflection Probes...", 1.0f);

            //iterate through all reflection probes in the scene again to assign them the file
            for (int i = 0; i < probes.Length; i++)
            {
                //bake each reflection probe with a progress bar
                string reflectionProbePath = sceneFolderPath + "/" + probes[i].name + ".exr";
                probes[i].bakedTexture = AssetDatabase.LoadAssetAtPath<Texture>(reflectionProbePath);
            }
        ...
        /// </summary>
        private void PlaceEmissives(bool saveEmissives = false)
        {
            if (saveEmissives)
                specularProbesParent = new GameObject(emissivesParentName);
            else
            {
                if (specularProbesParent != null)
                    DestroyImmediate(specularProbesParent);
            }
        ...
        private void SetupMesh(GameObject meshObject, Light light, float multiplier, Vector3 size)
        {
            ...
            if(specularProbesParent != null)
            {
                meshObject.transform.SetParent(specularProbesParent.transform);
            }
