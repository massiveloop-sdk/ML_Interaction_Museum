...
{
    public class FilterAdditiveBloom
    {
        private static int guiSectionSpacePixels = 10;

        private ComputeShader computeShader;
        private FilterAdditiveBloomOptions options;

        private Texture2D temp;

        public FilterAdditiveBloom()
        {
            computeShader = (ComputeShader)AssetDatabase.LoadAssetAtPath("Assets/Editor/TextureTools/Filters/AdditiveBloom/AdditiveBloom.compute", typeof(ComputeShader));
        }

        public void ShowUI()
        {
            if (options == null)
                options = new FilterAdditiveBloomOptions();
            ...
            EditorGUILayout.BeginVertical();
            options.threshold = EditorGUILayout.FloatField("Bloom Threshold", options.threshold);
        ...
        public void Render(TextureFileEntry textureFileEntry)
        {
            ...
            int compute_pass1 = computeShader.FindKernel("AdditiveBloom_Pass1");
            int compute_pass2 = computeShader.FindKernel("AdditiveBloom_Pass2");
            int compute_pass3 = computeShader.FindKernel("AdditiveBloom_Pass3");
            int compute_pass4 = computeShader.FindKernel("AdditiveBloom_Pass4");
            ...
            computeShader.SetFloat("BloomThreshold", options.threshold);
