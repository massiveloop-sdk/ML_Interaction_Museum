...
{
    public class FilterGaussianBlur
    {
        private static int guiSectionSpacePixels = 10;

        private ComputeShader computeShader;
        private FilterGaussianBlurOptions options;

        private Texture2D temp;

        public FilterGaussianBlur()
        {
            computeShader = (ComputeShader)AssetDatabase.LoadAssetAtPath("Assets/Editor/TextureTools/Filters/GaussianBlur/GaussianBlur.compute", typeof(ComputeShader));
        }

        public void ShowUI()
        {
            if (options == null)
                options = new FilterGaussianBlurOptions();
        ...
        {
            //create a temp RTs so the compute shader can write to it
            ...
            int compute_pass1 = computeShader.FindKernel("GaussianBlur_Pass1");
            int compute_pass2 = computeShader.FindKernel("GaussianBlur_Pass2");
            ...
            computeShader.SetTexture(compute_pass1, "Read_SourceTexturePass1", textureFileEntry.sourceTexture);
            ...
            computeShader.SetTexture(compute_pass2, "Read_SourceTexturePass2", tempRT_1);
