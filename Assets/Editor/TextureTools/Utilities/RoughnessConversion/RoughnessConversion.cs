...
{
    public class RoughnessConversion
    {
        private static int guiSectionSpacePixels = 10;

        private ComputeShader computeShader;
        private RoughnessConversionOptions options;

        private Texture2D temp;

        public RoughnessConversion()
        {
            computeShader = (ComputeShader)AssetDatabase.LoadAssetAtPath("Assets/Editor/TextureTools/Utilities/RoughnessConversion/RoughnessConversion.compute", typeof(ComputeShader));
        }

        public void ShowUI()
        {
            if (options == null)
                options = new RoughnessConversionOptions();
            ...
            EditorGUILayout.BeginVertical();
            options.mapping = (RoughnessMapping)EditorGUILayout.EnumPopup("Roughness Conversion", options.mapping);
