...
float3 UnityGI_Irradiance(ShadingParams shading, float3 tangentNormal, out float occlusion, out Light derivedLight)
{
    ...
    derivedLight = (Light)0;
        ...
        float3 sh_color = Irradiance_SphericalHarmonicsUnity(shading.normal, shading.ambient, shading.position);
        irradiance = sh_color;
        ...
        sphericalHarmonicsL1_direction += unity_SHBr.xyz + unity_SHBg.xyz + unity_SHBb.xyz + unity_SHC;
        sphericalHarmonicsL1_direction = normalize(sphericalHarmonicsL1_direction);

        derivedLight.colorIntensity = float4(sh_color, 1.0);
        derivedLight.l = sphericalHarmonicsL1_direction;
        derivedLight.attenuation = 1.0f;
        derivedLight.NoL = saturate(dot(shading.normal, sphericalHarmonicsL1_direction));

        occlusion = saturate(length(sh_color) * getExposureOcclusionBias());
    #endif
